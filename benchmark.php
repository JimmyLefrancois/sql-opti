 ...
 * - PHP 7.4+
 * - Extension PDO (pdo_mysql pour MySQL ou pdo_sqlsrv pour MSSQL)
 * - MySQL/MariaDB, MSSQL/SQL Server ou SQLite
 * 
...
const DB_TYPE = 'mssql'; // 'mysql', 'mssql' ou 'sqlite'
const DB_HOST = 'localhost';
const DB_PORT = 1433; // 3306 pour MySQL, 1433 pour MSSQL
    ...
    private function connectDatabase(): void
    {
        try {
            if ($this->dbType === 'sqlite') {
                ...
                // MSSQL : sqlsrv:Server=localhost,1433;Database=test_optimisation
                $dsn = sprintf(
                ...
                echo "📦 Connexion à MSSQL : " . DB_HOST . ":" . DB_PORT . "/" . DB_NAME . "\n";
            } else {
                // MySQL
                $dsn = sprintf(
...
$count}/{$totalBatches}\n";
    ...
    public function testUpdateOptimise(int $count): void
    {
        ...
        if ($this->dbType === 'mssql') {
            $this->pdo->exec("UPDATE collaborateur SET email = LOWER(prenom) + '.' + LOWER(nom) + CAST(id AS VARCHAR) + '@company.com'");
        } else {
        ...
        foreach ($batches as $batch) {
            ...
            if ($this->dbType === 'mssql') {
                // ✅ MSSQL : table temporaire locale avec #
                $this->pdo->exec("CREATE TABLE #temp_updates (\
            ...
            $tempTableName = $this->dbType === 'mssql' ? '#temp_updates' : 'temp_updates';
            $sql = "INSERT INTO {
$tempTableName} (matricule, salaire, email) VALUES " . implode(', ', $values);
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute($params);
            $queryCount++;
            ...
            // Nettoyer
            if ($this->dbType === 'mssql') {
                $this->pdo->exec("DROP TABLE #temp_updates");
            } else {
    ...
    {
        if ($this->dbType === 'mssql') {
            // ✅ MSSQL : DELETE FROM au lieu de TRUNCATE (plus sûr avec les FK)
            $this->pdo->exec("DELETE FROM collaborateur");
            $this->pdo->exec("DELETE FROM metier");
            $this->pdo->exec("DELETE FROM contrat");
        } else {
